...
public class CloseAttackWeapon : Weapon
{
    ...
    void Update()
    {
        ...
        if(attackCounter <= 0)
        {
            ...
            var Obj = Instantiate(damager,damager.transform.position,damager.transform.rotation,transform);
            ...
            for(int i = 1; i < stats[weaponLevel].amount; i++)
            {
                ...
                var obj = Instantiate(damager, damager.transform.position, Quaternion.Euler(0f,0f,damager.transform.rotation.eulerAngles.z + rot), transform);
                obj.transform.localPosition = Vector3.zero;
                obj.gameObject.SetActive(true);
